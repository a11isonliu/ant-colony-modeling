...
STEP_SIZE = 7
STEP_GRID = cp((-1*STEP_SIZE,0,STEP_SIZE),(-1*STEP_SIZE,0,STEP_SIZE))
...
class Nest:
    ...
    def __init__(self, canvas):
        ...
        self.posx = 150
        self.posy = 150
        self.display = circle(self.posx, self.posy, 15, canvas, "#F27E1D")
...
class Food:
    ...
    def replace(self, canvas):
        ...
        old_posx = self.posx
        old_posy = self.posy
        self.posx = randrange(50, 250)
        self.posy = randrange(50, 250)
...
class Pheromone:
    ...
    def __init__(self, ant, canvas):
        ...
        self.posx = ant.posx
        self.posy = ant.posy
        self.life = 10  # Life expectancy of the pheromone which expires after a certain time
...
class Environment:
    ...
    def __init__(self, ant_number):
        ...
        self.food1 = Food(self.environment, 250, 150)
        self.food2 = Food(self.environment, 50, 150)
        ...
        self.environment.after(
            0.01, f_move(self.environment, self.ant_data, self.food1, self.food2))
        self.root.mainloop()
...
def collide(canvas, ant):
    ...
    if canvas.find_overlapping(ant_coords[0], ant_coords[1], ant_coords[2], ant_coords[3])[0] == 1:
        ...
        return 2
    elif canvas.find_overlapping(ant_coords[0], ant_coords[1], ant_coords[2], ant_coords[3])[0] == 3:
        return 3
    else:
        return 0
...
def pheromones_affinity(ant, canvas):
    ...
    ant_coords = (ant.posx, ant.posy)
    HG = len(canvas.find_overlapping(
        0, 0, ant_coords[0], ant_coords[1])) - (1 + nb_ant)
    HD = len(canvas.find_overlapping(
        e_w, 0, ant_coords[0], ant_coords[1])) - (1 + nb_ant)
    BG = len(canvas.find_overlapping(
        0, e_h, ant_coords[0], ant_coords[1])) - (1 + nb_ant)
    BD = len(canvas.find_overlapping(
        e_w, e_h, ant_coords[0], ant_coords[1])) - (1 + nb_ant)
    new_move_tab = []
...
def f_move(canvas, ant_data, food1, food2):
            ...
            if ant.scout_mode:  # if the ant is looking for a food source
                ...
                if n == 2 or n == 3:
                    ...
                    if n==2:
                        ...
                        if food1.life < 1:
                            food1.replace(canvas)
                    if n==3:
                        food2.life -=1   
                        if food2.life < 1:
                            food2.replace(canvas)
